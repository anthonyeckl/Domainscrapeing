...
class GodaddyDb():
    ...
    def insert_domain(self, domain_info_list):
        ...
        if self.check_domain(domain_info_list[0]) == 0:
            self.c.execute(
               "INSERT INTO godaddy_auction_scrape VALUES (:name, :ext, :bids, :traffic, :estimatedvalue, :price, :time, :timedate)",
                {"name": domain_info_list[0], "ext": domain_info_list[1], "bids":domain_info_list[2], "traffic":domain_info_list[3], "estimatedvalue":domain_info_list[4], "price":domain_info_list[5], "time":domain_info_list[6], "timedate":datetime.datetime.now()})
            #self.c.execute(
...
#tst2 = [['mommyrunfast', '.com', '9', '11', '910', '349', '1D 5H ']]
#gdb = GodaddyDb("godaddyauction.db")
#gdb.insert_domain(tst2[0])
#gdb.initiate_db()
#gdb.print_db()
#gdb.close_db()
